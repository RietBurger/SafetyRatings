...
namespace SafetyRatings.Controllers
{
    public class SafetyController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            // oAuth Token
            var token = "GnKcGPeEXWsfUq5uySZA7Q6oe3CT";
            var latitude = "41.397158";
            var longitude = "2.160873";

            // see if token is still valid. if not, get new token
            token = Check_token_validity(ref token);

            // Call API and return response
            IRestResponse response = get_info(ref token, ref latitude, ref longitude);
        ...
        public IRestResponse get_info(ref string token, ref  string longitude, ref string latitude)
        {
            //var baseUrl = new Uri("https://test.api.amadeus.com/v1/safety/safety-rated-locations?latitude=" + latitude + "&longitude=" + longitude + "&radius=2");
